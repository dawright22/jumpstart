...
resource  "null_resource" "backend_file" {
  ...
  command =  "echo  workspaces '{' name = \\\"${tfe_workspace.quickstart-workspace.name}\\\" '}' >> ./module/quickstart/backend.hcl"
  }
  provisioner "local-exec" {
  command =  "echo hostname = \\\"app.terraform.io\\\" >> ./module/quickstart/backend.hcl"
  }
  provisioner "local-exec" {
  command =  "echo  organization = \\\"${tfe_organization.quickstart-org.name}\\\" >> ./module/quickstart/backend.hcl"
  }
}
